...
docker run -d -p 2222:2222 -v /var/run/docker.sock:/var/run/docker.sock -v /var/ssh/:/ssh_keys -v /:/host -e PUBLIC_KEY_FILE=/ssh_keys/authorized_keys -e USER_NAME=dev -e SUDO_ACCESS=true -e HOSTNAME=$HOSTNAME --name sshd lscr.io/linuxserver/openssh-server
